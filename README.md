...
**품질 보증:** 테스트 292개. 전부 "정답을 손으로 계산할 수 있는 문제"로 검증
...
| **실선 생성기(ShipGen)** | Ship-D 45파라미터가 정식 생성기로 — 조건(짐·속도)이 문법을 결정(규칙 0줄), NSGA 부분공간이 수식 전선에 12승 0패, 파이프라인은 실척 vs 수식 실측 A/B로 이긴 쪽 채택 (`--hull-source auto\|shipd\|formula`) |

물리 검증 원칙은 그대로: 모든 모듈이 해석해·문헌·독립 구현 대조를 통과해야 병합 (테스트 292개).
