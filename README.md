...
**품질 보증:** 테스트 418개. 전부 "정답을 손으로 계산할 수 있는 문제"로 검증
...
| **NSGA × 8중 게이트 (캠페인 완주)** | 선형(Ship-D 45)+속도를 NSGA-II로 동시 최적화, 평가 = 8중 게이트 사슬 — 1.28만 평가 대예산. 발견: "친환경 = 저비용" (EEDI·단가 정렬 — 목적을 단가 vs 속도로 재구성), 엔진 계단이 전선 형태 결정, **내항 게이트가 fast 전선 50척 중 47척 학살** (2단 구도 존재 이유 실증). 진짜 8중 전선: 경제형 9.9 kn·단가 0.406 $/kt·nm (격자 대비 2.5배 개선) / 속도형 14.6 kn. 정직 각주: 대형 화물창 용적 게이트 부재·조종 Cb<0.51 미검 (백로그 최우선) (`src/ai/optimize_large.py`) |
...
물리 검증 원칙은 그대로: 모든 모듈이 해석해·문헌·독립 구현 대조를 통과해야 병합 (테스트 418개).
