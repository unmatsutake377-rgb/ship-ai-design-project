...
**품질 보증:** 테스트 436개. 전부 "정답을 손으로 계산할 수 있는 문제"로 검증
...
물리 검증 원칙은 그대로: 모든 모듈이 해석해·문헌·독립 구현 대조를 통과해야 병합 (테스트 436개).
