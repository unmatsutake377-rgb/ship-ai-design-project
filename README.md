...
**품질 보증:** 테스트 335개. 전부 "정답을 손으로 계산할 수 있는 문제"로 검증
...
| **파랑·내항성 (진행 중)** | 스트립 이론 정통 캠페인 — Lewis 단면 등각사상(반원 해석 앵커)·메쉬 스테이션 추출(σ=Cm 앵커)·횡요 고유주기(IMO 규정식 — 100 m 화물선 14.9 s)까지. 목표: 5번째 게이트 "이 파도에서 버티나" (`src/physics/seakeeping/`) |
...
물리 검증 원칙은 그대로: 모든 모듈이 해석해·문헌·독립 구현 대조를 통과해야 병합 (테스트 335개).
