...
**품질 보증:** 테스트 321개. 전부 "정답을 손으로 계산할 수 있는 문제"로 검증
...
| **전 크기 개방** | "크기는 사용자의 축" — 소형 USV와 **100 m 화물선이 같은 CLI**: cargo 용도 → Holtrop-Mennen 저항 (KCS 공개 벤치마크 대조) + Watson 경하 + Wageningen B 프로펠러 실설계 (공개 계수·캐비테이션 검사) + 엔진 카탈로그 + IMO GM·ICLL 건현 게이트. 예: `--payload 5000000 --purpose cargo --endurance 240` → DWT 5,000 t급 합격 설계 |
...
물리 검증 원칙은 그대로: 모든 모듈이 해석해·문헌·독립 구현 대조를 통과해야 병합 (테스트 321개).
