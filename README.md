...
| 순서 | 이름 | 내용 | 끝나면 얻는 것 |
|---|---|---|---|
| 다음 | **M4a** 동역학 계수 | 배가 움직일 때의 "물의 저항 성질" 숫자화 (부가질량 등) | 움직임 시뮬레이션 재료 |
| | **M4b** 웨이포인트 시뮬 | 좌우 모터 2개 + 자동 조타(LOS+PID)로 지정 지점 순회 | **배가 처음으로 움직임** — 궤적 그림 |
| | **M5a** 실제 데이터셋 | MIT Ship-D (배 3만 척 데이터) 연동 | 진짜 배 형상 다양성 |
| | **M5b** AI 학습 | 물리 계산 결과를 정답지로 신경망 학습 + 당신 점수 반영 | 원안의 "AI 설계" 완성 |
| | **Phase B** ROS2 연동 | Docker로 로봇 시뮬레이터(Gazebo/VRX)에 배 투입 | 자율운항 알고리즘 테스트베드 |
| | **Phase C** 고속선·CFD | 빠른 보트(활주형) + 정밀 유체해석 | 전체 속도 영역 커버 |
