...
**품질 보증:** 테스트 429개. 전부 "정답을 손으로 계산할 수 있는 문제"로 검증
...
| **쌍동선 (1단계 개통)** | 데미헐 2개 병합 기하 + **기존 정역학이 평행축 정리를 자동 실측** (수선면 폐곡선 합산 구조 — 새 정역학 코드 0줄, 손계산 1% 재현) + 데미헐×2 저항 (간섭 무시 C급). BlueBoat급 실선 자릿수 정합 (흘수 7.2 cm·GM 1.1~2.9 m), 단동 대비 GM 1.6배 실증. **2단계: `--catamaran` 파이프라인 통합** — 같은 조사 임무에서 단동 대비 저항 −59%·GM 2배 실증 (날씬 데미헐 조파 급감 + 평행축). 스캔라인 짝수 교차 일반화로 구조 하중도 쌍동 대비. 정직 스킵 2종 (내항 단동 단면 가정 밖·space 크로스데크 탑재). 후속: 쌍동 내항 물리·데크 모델·Molland 간섭 원전 (`src/ai/catamaran.py`) |
...
물리 검증 원칙은 그대로: 모든 모듈이 해석해·문헌·독립 구현 대조를 통과해야 병합 (테스트 429개).
