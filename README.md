...
**품질 보증:** 테스트 432개. 전부 "정답을 손으로 계산할 수 있는 문제"로 검증
...
| **NSGA × 8중 게이트 (캠페인 완주)** | 선형(Ship-D 45)+속도를 NSGA-II로 동시 최적화, 평가 = 8중 게이트 사슬 — 1.28만 평가 대예산. 발견: "친환경 = 저비용" (EEDI·단가 정렬 — 목적을 단가 vs 속도로 재구성), 엔진 계단이 전선 형태 결정, **내항 게이트가 fast 전선 50척 중 47척 학살** (2단 구도 존재 이유 실증). 진짜 8중 전선: 경제형 9.9 kn·단가 0.406 $/kt·nm (격자 대비 2.5배 개선) / 속도형 14.6 kn. **v2 (roll 프록시 재진화)**: 전선 97척·Cb 0.50 자기 수렴·roll 1° 이하 — 단, pitch 3.0~3.3° 초박빙 (C급 한계 3.0° vs 3.5° 가정 시 전원 합격) → "기준 정밀도가 판정을 지배하는 지점" 검출, pitch 기준 승급 최우선 백로그. 정직 각주 해소: **대형 화물창 용적 게이트 신설** (메쉬 실측 절단 — 이중저·기관실·연료 공제, 적재계수 1.3 m³/t) → 전선 3척 재판정 전부 생존 (용적 여유 ~3배 — "미검" 각주가 실측 합격으로 승격). 조종 추정 슬렌더 외삽(Cb 0.40~0.51 C급) 확장으로 **경제형 전선 = 스킵 0개 완전 검증 8중 합격** (D_T 4.46L·OSA 4.3° IMO 실검). 속도형(Cb 0.378)만 외삽 한계 밖 정직 스킵 (`src/ai/optimize_large.py`·`src/physics/cargo_capacity.py`) |
...
물리 검증 원칙은 그대로: 모든 모듈이 해석해·문헌·독립 구현 대조를 통과해야 병합 (테스트 432개).
