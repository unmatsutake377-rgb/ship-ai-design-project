...
> 갱신: 2026-07-26 · 테스트 56개 통과 · main 브랜치 기준
> 매 세션 시작할 때 이 파일부터.

## 지금 어디인가 (한 문장)

**Phase A 전반부 완료** — "목적 입력 → 배 설계 → 물리 검증 → 소요 추력 계산"까지 명령 한 줄로 동작. 다음은 오너 피드백 반영 3건, 그 뒤 배를 움직이게 만들기(M4).

## 파이프라인 모듈 현황

```mermaid
flowchart LR
    A[목적 입력<br>속도·적재량·용도] --> B[치수 추정<br>✅ 개략 / 🔧 개편예정]
    B --> C[속도 체계 판정<br>✅]
    C --> D[3D 선형 생성<br>✅ 공식 / M5에서 AI 교체]
    D --> E[무게·무게중심<br>✅ 개략 / M4a에서 분포모델]
    E --> F[정역학 검증<br>✅ / 🔧 종류별 밴드 예정]
    F --> G[저항·소요추력<br>✅]
    G --> H[모터 선택<br>⬜ 백로그 #16]
    H --> I[동역학 시뮬<br>⬜ M4a·M4b]
...
| 순서 | 작업 | 유래 | 오너 참여 |
|---|---|---|---|
| ① | 속도 실현가능 범위 함수 (hull speed 역산) | 오너 제안 Q2 | 결과 피드백 |
| ② | 실선 데이터 수집 → 치수 추정 데이터 기반 개편 + 종류별 흘수·GM 밴드 | 오너 제안 Q1·Q5 | **참고선 같이 선정** |
| ③ | 모터 카탈로그 선택 모듈 (실제 스러스터 제원) | 오너 제안 Q6 | 후보 같이 검토 |
| ④ | M4a — Fossen 동역학 계수 (부가질량·미계수) + weights 분포모델 승격 (LCG·Izz) | 스펙 | 개념 학습 |
| ⑤ | M4b — LOS+PID 차동추력 웨이포인트 시뮬 | 스펙 | 궤적 평가 |
...
- 치수 비율·중량 상수는 개략값 — ②③에서 실측 데이터로 교체 예정
