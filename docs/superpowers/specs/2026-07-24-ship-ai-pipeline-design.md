...
- 날짜: 2026-07-24 (v2 — 적대적 설계 검토 반영) / 2026-07-27 (v3 — M5 재편, §7)
- 상태: v1 사용자 승인 → 독립 검토 13건 반영(v2) → M5 방향 오너 결정 반영(v3)
...
## 7. M5 재편 — 확산모델 대신 다목적 최적화 + 대리모델 (v3, 2026-07-27 오너 결정)

**배경:** ShipGen(확산) 검토 결과, 우리 목적("목적 → 최적 배")은 다양성 생성이 아니라
요구조건 만족 최적해 탐색 — 산업 표준인 GA+대리모델이 적합. 우리는 물리 평가가
빠르므로(Michell ~초) 대리모델이 필수가 아니라 가속 수단이라는 이점도 있음.

**오너 결정 3건:**
1. **다목적 파레토 (NSGA-II)** — 저항·안정성(GM 여유)·크기/중량의 트레이드오프를
   파레토 전선으로 제시, 최종 선택은 사람. 단일 저항 최소화 배제.
2. **HITL을 쌍대비교 ELO로 교체** — "두 배 중 어느 쪽?"이 1~5점보다 초보 평가자에게
   신뢰성 높음 (Q7 불안 해소). 기존 점수 로깅은 보존, 주 인터페이스만 교체.
   (오너의 ELO 제안이 안착한 자리. GA 토너먼트 선택도 같은 사상임을 확인함)
3. **설계 공간 단계적 확장** — Wigley 파라미터 공간에서 전체 기계(대리모델·NSGA-II·
   ELO) 검증 → Ship-D 45파라미터 공간으로 교체. 최적화기는 "벡터→메쉬→평가"
   인터페이스만 보므로 공간 교체 = 생성기 교체.

**재편된 M5:**
| 단계 | 내용 |
|---|---|
| M5-ELO | 쌍대비교 ELO 모듈 (K=32, 초기 1500) + 비교 이력 CSV |
| M5a | Ship-D 로더 + 45파라미터 형상 재구성 (다운로드 오너 승인 필요) |
| M5b-1 | 대리모델: 우리 물리 라벨(저항·GM)로 MLP 학습 (Mac MPS) |
| M5b-2 | NSGA-II 다목적 최적화 — 1차: Wigley 공간 + 물리 직접 평가, 2차: 대리모델 가속 + Ship-D 공간 |
| 보류 | 확산모델(ShipGen 방식) — 다양성 탐색이 필요해질 때 재검토 |
