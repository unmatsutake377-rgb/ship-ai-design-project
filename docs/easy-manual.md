...
> (마지막 갱신: 2026-08-07 — 17일차 완결: 수집 대회전과 파도의 시작)
...
추출기(중앙 단면 풍만도가 설계값과 일치하는지로 검증). 다음
벽돌은 "파도를 만들며 잃는 에너지" — 원전 문헌을 확보한 뒤에
갑니다 (기억으로 공식을 쓰지 않는 원칙).
...
**참고 숫자** (이해가 먼저, 숫자는 곁들임): 자동 검증 시험 332개 전부 통과 ·
