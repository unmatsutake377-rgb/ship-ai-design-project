...
**참고 숫자** (이해가 먼저, 숫자는 곁들임): 자동 검증 시험 270개 전부 통과 ·
가상 바다(직립 정직화 완료) 완주 조타 3구성 · AI 학습 라벨 1만 장 —
특징 v3로 저항 예측 +23%, 실전 합격률 81% · 실선 데이터 12척 + 화물
실측 3점 + 모터 5종 + 방향타·서보 공식 스펙 · 정밀해석(CFD) 20여 회 ·
실패 박물관 26건 · 코드·기록 전부 GitHub 공개

**다음에 할 일 후보** (오너 결정 순서): 활주정 직립 재계측 / AI 안정
과목 전용 특징 (특징 v3의 유일한 회귀 과목) / 실선 데이터 확충 /
**그 뒤에** ELO 대결 재개 (시뮬 자산 다 쌓인 뒤 몰아보기 — 오너 결정)
