...
> (마지막 갱신: 2026-08-12 — 표준 정본으로 재료를 다시 세운 날)
...
4¾. [여덟 개의 관문 — 배가 통과해야 하는 시험](#4¾-여덟-개의-관문--배가-통과해야-하는-시험-프로젝트의-심장) ⭐
...
[설계 나선]  무게 추정 → 얼마나 잠기나 → 물의 저항 → 필요한 모터·배터리
   ...
   ↓
[합격 검사 = 여덟 관문]  뜨나·안 넘어지나·실리나·안 흔들리나·
                 안 부러지나·돌 수 있나·규제 통과하나·탄소 성적
                 (하나라도 떨어지면 그 배는 탈락 — 아래 §4¾에 쉬운 설명)
   ↓
...
### 실무 설계 순서와의 대응 — 우리는 "설계 나선"입니다

조선소 실무의 기본설계 순서는 **선행(주요치수·선형) → 배치(구획·
중량) → 계산(정역학·복원성) → 구조(강도) 진행, 그리고 저항은
전 과정 상시(all time)** 로 돌아갑니다. 우리 조립 라인이 정확히
이 순서의 각 단계를 갖추고 있습니다:

| 실무 순서 | 우리 파이프라인 |
|---|---|
| 선행 (주요치수·선형) | 치수 추정(실선 통계) → 3D 선형 생성 |
| 배치 (구획·중량) | 짐칸 공간 검사 + 중량 분해 + 짐 배치 처방 |
| 계산 (정역학·복원성) | 부력·GM·트림 + 설계 나선 수렴 |
| 구조 (강도) | 여섯 번째 관문 (규정 하중·좌굴·보강재) |
| **저항 (상시)** | **나선 매 반복마다 재계산** — 무게가 바뀌면 흘수가, 흘수가 바뀌면 저항이, 저항이 바뀌면 모터가 바뀌는 꼬리물기를 수렴시킴 |

한 가지 차이: 실무는 사람이 하니 대체로 **한 단계씩 순차**지만,
우리는 컴퓨터라 **모든 단계를 되먹임 고리로 묶어 동시에 수렴**
시킵니다 — 조선공학 교과서의 "설계 나선(design spiral)" 개념
그대로입니다. 거기에 우리만의 세 층을 얹었습니다: ① 여덟 관문
자동 판정(사람이 하던 규정 대조를 자동으로) ② 진화(한 척을
다듬는 대신 수만 척을 걸러 최적 전선) ③ 정직 검수(값마다 출처
등급, 모르면 모른다고). 범위는 기본설계(basic design)까지 —
상세 배치(배관·의장)는 실무 상세설계 영역이라 이 실험 밖입니다.
...
## 4¾. 여덟 개의 관문 — 배가 통과해야 하는 시험 (프로젝트의 심장)

이 프로젝트의 핵심은 한 문장입니다: **"설계한 배가 여덟 개의
물리·규제 관문을 전부 통과해야 진짜 합격."** 하나라도 떨어지면
탈락입니다. 처음엔 4개였는데, 회차를 거치며 하나씩 늘어 지금은
여덟 개입니다. 각 관문이 묻는 것을 일상어로 옮기면:

| # | 관문 | 한 문장 질문 | 떨어지면 |
|---|---|---|---|
| 1 | **뜨나** | 무게만큼 정확히 뜨고, 뱃전이 물 위로 충분히 나오나? | 가라앉거나 물이 넘침 |
| 2 | **안 넘어지나** | 기울여도 되돌아오나 (복원력 GM이 적당한가)? | 뒤집히거나 멀미나게 흔들림 |
| 3 | **실리나** | 짐이 실제로 짐칸 공간에 들어가나? | 무게는 되는데 부피가 안 됨 |
| 4 | **안 흔들리나** | 파도 속에서 사람이 일할 만큼만 흔들리나 (내항성)? | 옆흔들림·앞뒤흔들림 과도 |
| 5 | **안 부러지나** | 파도가 배를 휘게 할 때 선체가 버티나 (종강도)? | 부러지거나 갑판이 주저앉음(좌굴) |
| 6 | **돌 수 있나** | 타를 꺾으면 규정 안에서 도나 (조종성)? | 선회 반경이 너무 커 못 피함 |
| 7 | **규제 통과하나** | 국제 탄소 규제(EEDI) 설계 상한을 지키나? | 규제 위반으로 취항 불가 |
| 8 | **탄소 성적** | 실제 운항 탄소 등급(CII A~E)이 몇 등급인가? | (성적표 — 합불 아님) |

관문 1·2·3은 잔잔한 물에서의 기본기, 4·5·6은 거친 바다·실제
운항에서의 생존력, 7·8은 돈과 법의 언어입니다. 배 하나가 나올
때마다 이 여덟 개를 전부 통과하는지 자동으로 검사하고, 떨어지면
**왜** 떨어졌는지 정직하게 알려줍니다 (데이터가 없어 검사 못 하면
"불합격"이 아니라 "정직 스킵"으로 표시 — 모른다고 떨어뜨리지
않는 것도 원칙입니다).

> 왜 관문이 중요한가: AI에게 "제일 싼 배를 만들라"고만 하면,
> AI는 규칙의 빈틈으로 도망쳐 "종이 위에선 싸지만 실제로는
> 뒤집히거나 못 도는 배"를 내놓습니다. 관문은 그 도망칠 곳을
> 막는 심판입니다. 이 프로젝트의 여러 날이 "진화가 심판 없는
> 곳으로 도망쳤다 → 새 심판을 세웠다"의 반복입니다 (§6 참고).
...
| **설계 나선 (design spiral)** | 무게↔잠김↔저항↔모터가 서로 물고 도는 것을 답이 안 변할 때까지 반복하는 과정. 실무 기본설계 순서(선행→배치→계산→구조, 저항 상시)를 되먹임 고리로 묶은 것 — §4 대응표 참조 |
...
| **내항성** | 파도 속에서 배가 얼마나 버티고 일하나 — 잔잔한 물 성적과 별개의 과목 (4번째 관문, 완성됨) |
...
| **호깅 / 새깅** | 파도 위에서 배가 휘는 두 모양. 호깅(∩)=가운데가 들림→갑판 늘어나고 바닥 눌림. 새깅(∪)=가운데가 처짐→갑판 눌리고 바닥 늘어남 |
| **좌굴 (buckling)** | 얇은 판이 눌릴 때 **찢어지기 전에 먼저 우그러지는(주저앉는)** 현상. 콜라캔을 위에서 누르면 찌그러지는 것과 같음 — 압축받는 갑판/바닥이 항복(찢김)보다 먼저 당하는 경우가 많음 |
| **종강도** | 배 전체를 하나의 긴 막대(보)로 봤을 때, 파도가 휘게 하는 힘을 버티는 힘. 5번째 관문 |
| **EEDI** | 배의 "설계 탄소 연비" — 짐 1톤을 1해리 나를 때 나오는 CO₂. 국제 규제가 상한을 정함 (7번째 관문) |
| **CII** | 배가 **실제 운항**에서 받는 탄소 성적 (A~E 등급). 매년 받는 통지표 (8번째 관문 — 합불 아닌 성적) |
| **감속 운항 (slow steaming)** | 속도를 조금 낮추면 연료가 크게 줄어드는 것 (저항은 속도의 세제곱). 규제 통과와 연료비 절감이 같은 방향 |
| **조종성 / MMG** | 배가 얼마나 잘 도나 (6번째 관문). MMG는 배의 회전을 예측하는 조선공학 표준 힘 모델 (실측 유조선 KVLCC2로 검증) |
| **선회지름** | 타를 끝까지 꺾었을 때 배가 그리는 원의 지름 (배 길이의 몇 배). 국제 규정은 5배 이내를 요구 |
| **프록시 (proxy)** | 무거운 계산의 결과를 **가볍게 미리 맞히는 대역 계산**. 예: 몇천 걸음 시뮬레이션 대신 방정식 한 번으로 선회지름 근사 — 수만 척을 빠르게 거를 때 씀 |
| **진화 (NSGA)** | 수만 척의 배를 세대를 거쳐 "더 나은 것만 살아남기"로 개량하는 최적화. 관문이 심판, 진화가 도전자 |
| **파레토 전선** | 진화가 찾아낸 "어느 쪽으로도 지지 않는" 최적 배들의 띠 (예: 싸지만 느린 배 ~ 빠르지만 비싼 배) |
| **수송단가** | 짐 1톤을 1해리 나르는 데 드는 연료비 ($/t·nm). 진화가 최소화하려는 목표 |
| **RMS / 유의값** | 흔들림을 재는 두 잣대. RMS=평균적 흔들림, 유의값=큰 흔들림 대표값(≈RMS의 2배). 국제 기준이 어느 쪽인지 헷갈리면 2배 엄격/느슨해짐 (30일차 사건) |
| **백지 리뷰** | 코드를 검토하는 AI에게 **내 설명·의도를 안 주고 코드와 목표만** 주는 것. 내 잘못된 가정에 물들지 않은 독립 검사 — 실제로 내 부호 실수를 잡음 (37일차) |
| **하드 게이트 / 성적표** | 하드 게이트=떨어지면 탈락하는 진짜 관문. 성적표=점수만 매기고 탈락은 안 시키는 참고용. 새 검사는 성적표로 먼저 넣어보고 확신 서면 하드 게이트로 승격 |
| **nohup 배경 완주** | 오래 걸리는 계산을 대화창과 분리된 독립 프로세스로 돌려, 세션이 끊겨도 끝까지 돌게 하는 방식 (단, 노트북이 잠들면 멈춤 — 37일차 교훈) |
...
수식을 글자로 뽑으면 숫자 순서가 뒤섞이는 문제가 있어서, **페이지를
이미지로 그려 눈으로 읽는** 방식으로 바꿨습니다. 추정 계수로
...
### 8/11 (33일차) — 전시장이 문을 열었다

한 달 동안 만든 것들이 전부 터미널 글자로만 존재했는데, 오늘
브라우저 화면이 생겼습니다. **라이브 설계**: 용도·속도·짐 세
개만 고르고 버튼을 누르면 배가 설계되는 과정이 실황으로 흐르고,
여덟 관문의 합격/불합격/스킵이 신호등으로, 배 모양이 돌려볼 수
있는 3D로 뜹니다. **갤러리**: 진화가 찾아낸 완전 검증 70척의
지도, 한 달의 연대기, 파도 타는 배 영상까지.

원칙은 화면에도 그대로: 불합격은 빨갛게 정직하게, 스킵은 "데이터
없음 ≠ 불합격"이라 적고, 자산이 없으면 없다고 안내합니다. 물리
코드는 한 줄도 새로 쓰지 않았습니다 — 화면은 화면일 뿐, 판정은
시험 447개가 지키는 기존 엔진이 합니다. 인터넷 공개는 하지 않는
로컬 전시장입니다 (데이터 라이선스 존중).

### 8/11 (34일차) — AI에게 알맞은 일을 나눠 준 날

일을 **여러 AI에게 능력에 맞게 나누는** 체계를 세웠습니다. 사람도
어려운 판단은 전문가가, 심부름은 신입이 하듯 — 오탈자·검색은 작고
빠른 모델, 원전 찾기·화면 배선은 중간 모델, 물리 판단·서사는 큰
모델. 특히 **"판단은 항상 내가 직접 한다"**를 원칙으로 박았습니다:
심부름꾼이 후보를 모아 와도 채택·기각은 메인이 결정 (실제로
이날 실선 후보 8척을 받아 1척만 채택). 이 배분을 처음 실전에서
4작업 병렬로 돌려봤고, 놀랍게도 심부름꾼이 자기가 못 미더운 값을
스스로 "이건 의심스럽다"고 정직하게 표시해 줬습니다 — 정직 원칙이
AI끼리도 통한 겁니다.

### 8/11 (35일차) — 여덟 후보, 살아남은 건 하나

실선 데이터를 더 모았습니다. 이번엔 AI 심부름꾼(중형 모델)이
인터넷에서 여덟 척의 후보를 찾아왔고, 제가 한 척씩 검수했습니다.
결과는 **여덟 중 하나만 통과.** 나머지 일곱을 왜 버렸는지가 이
프로젝트의 핵심입니다: 5m 배에 폭이 8m라고 적힌 후보(물리적으로
불가능 — 예전 "폭 환각" 사고와 똑같은 냄새), 만재 무게가 650kg인데
짐을 600kg 싣는다는 후보(그럼 배 자체가 50kg? 불가능), 자료마다
길이·무게가 다른 후보, 꼭 필요한 값이 아예 빠진 후보들. 통과한
한 척은 Voltari라는 **전기 순찰정**(배터리 168kWh) — 우리가
부족했던 "전기추진 경비정" 칸을 정확히 채웠습니다.

교훈: 데이터는 많이 모으는 게 아니라 **믿을 수 있는 걸 모으는**
겁니다. 심부름꾼이 자기가 찾은 값도 "이건 좀 의심스럽다"고 정직하게
표시해줬고(모델끼리도 정직 규칙이 통했습니다), 제가 최종 판정.
실선 18척 → 19척.

### 8/11 (36일차) — 항복은 통과, 그런데 주저앉는다

배가 "부러지나"를 보던 여섯 번째 관문(구조)에 빠진 게 있었습니다.
지금까지는 "재료가 늘어나 끊어지나"(항복)만 봤는데, 실제로는
**눌려서 주저앉는**(좌굴) 실패가 먼저 오는 경우가 많습니다. 얇고
넓은 갑판이 호깅(가운데가 들리는 파도)에서 눌릴 때가 그렇죠.

마침 규정 원전이 예전에 구해둔 IACS 문서 안에 이미 있었습니다
(S11.5 좌굴). 수식이 PDF에서 글자로 뽑으면 뒤섞여서, 페이지를
그림으로 렌더해 눈으로 읽어 옮겼습니다. 넣고 100m 화물선을
돌려보니: **갑판이 항복 기준은 통과(174 vs 한계 175)하는데
좌굴 기준은 크게 불합격(주저앉는 한계 59 vs 받는 힘 174).**
항복만 보던 관문이 이 위험에 눈이 멀어 있었던 겁니다.

이번엔 성적표로만 병기했습니다(합불엔 아직 안 넣음) — 좌굴까지
합격시키려면 갑판을 훨씬 두껍게/촘촘하게 해야 해서 배 무게와
설계 전반이 바뀌기 때문입니다. 그건 다음 숙제. 새 심판이 자기가
찾은 코드도 독립적으로 검토(다른 AI가)해 3건을 다듬었습니다.

### 8/11 (37일차) — 심판이 내 실수를 잡았다

어제 성적표로만 병기했던 좌굴(주저앉음) 검사를, 이번엔 진짜
합격 조건에 넣었습니다. 이제 갑판·선저가 눌려 주저앉지 않을
때까지 두껍게 키웁니다.

그런데 여기서 제가 실수를 했습니다. "배가 가운데가 들릴 때(호깅)
갑판이 눌린다"고 코드에 적었는데, 사실은 **반대**입니다 — 가운데가
들리면 갑판은 늘어나고(인장), 가운데가 처질 때(새깅) 갑판이
눌립니다. 게다가 새깅이 더 큰 힘이라, 제 실수대로면 진짜 위험한
갑판을 작은 힘으로 검사해 얇게 만들 뻔했습니다.

이 실수를 **다른 AI 심판(백지 리뷰어)이 잡았습니다.** 놀라운 건,
제가 짠 검사 시험조차 같은 실수를 담고 있었는데 — 심판에게는
제 시험을 안 보여주고 "코드와 목표"만 줬더니, 제 잘못된 가정에
물들지 않고 물리 오류를 찾아낸 겁니다. "내 가정이 내 시험까지
통과시켜도, 백지 심판은 통과 못 시킨다" — 이게 독립 검토를 하는
이유입니다. 고치니 100m 갑판이 8mm에서 16mm로 두꺼워졌습니다.

### 8/12 (38일차) — 표준 원본이 우리 실수를 두 개 잡았다

알루미늄 배 구조가 그동안 **추정값**으로 돌아가고 있었는데, 오너가
국제표준(ISO 12215-5)의 한국판(KS)을 무료 열람으로 찾아내 정본
숫자를 넘겨줬습니다. 자동 도구로는 로그인 벽·봇 차단을 못 뚫는
문헌을, 사람 브라우저가 여는 협업이 다시 통했습니다.

정본을 넣자 실수 두 개가 드러났습니다. 첫째, 우리는 알루미늄
**용접부 항복강도 125**를 "버틸 수 있는 한계"로 써왔는데, 표준은
설계에 실제로 써야 할 값은 그보다 낮은 **88**이라고 못박습니다
(용접하면 30% 더 보수적으로). 즉 알루 배를 1.4배 얇게 만들 뻔한
과소설계였고, 정직하게 88로 낮췄습니다.

둘째는 백지 리뷰(다른 AI 심판)가 잡았습니다: 알루미늄 판의 좌굴
(주저앉음)을 계산할 때 **강철의 탄성계수**를 잘못 쓰고 있었습니다
(알루는 강보다 3배 잘 휘는데). 이러면 좌굴을 3배 강하게 착각해
위험한 판을 통과시킵니다. 알루가 이제 관문을 제대로 통과하게
되면서 숨어 있던 이 버그가 드러났고, 바로 고쳤습니다. **세 번째
회차 연속으로 백지 심판이 진짜 물리 버그를 잡은 것**입니다.

### 8/12 (38일차 후속) — 뼈대 살(보강재)도 안 주저앉게

배 껍데기(판)가 눌려 주저앉는 것(좌굴)은 이미 관문으로 막았는데,
그 판을 뒤에서 받치는 **보강재(뼈대 살)의 웨브**도 얇고 높으면
주저앉습니다. 그동안 우리는 보강재를 "면적 덩어리 하나"로만 다뤄
그 세부 모양(웨브 높이·두께)이 없어 이 검사를 못 했습니다.

이번에 보강재의 실제 모양을 설계하는 모델을 세웠습니다: 필요한
강도를 내면서도 웨브가 "너무 높고 얇지 않게"(국제표준 KS ISO
12215-5의 세장비 한계) 만드는 최소 플랫바를 자동으로 찾습니다.
알루미늄은 강철보다 3배 잘 휘어서 웨브를 더 두껍게 만들어야
합니다(같은 강도라도) — 물리가 그대로 반영됐습니다.

백지 심판(다른 AI)이 "다른 응력값을 쓰라"고 제안했지만, 표준
원본 표의 공식과 미리 계산된 값을 역으로 풀어보니 우리가 쓴
값(용접 항복강도)이 맞았습니다 — 근거를 들어 정중히 기각했습니다.
심판의 지적도 무조건 따르는 게 아니라 원전으로 되짚는 게 이
프로젝트의 방식입니다. 대신 심판이 잡은 진짜 버그 두 개(거대한
경우의 치수 오염 등)는 바로 고쳤습니다.

### 8/12 (38일차 마무리) — 실선처럼 T자 뼈대로

방금 만든 보강재는 "납작한 막대(플랫바)"였는데, 진짜 배들은
**T자 모양** 뼈대를 씁니다. 왜일까요? T자는 위에 가로 날개(플랜지)가
있어 세로 판(웨브)을 잡아줘서, 웨브를 훨씬 얇고 높게 만들어도
안 주저앉습니다 (국제표준 기준으로 3배 이상 슬렌더 허용). 그래서
같은 강도를 **24% 더 가볍게** 낼 수 있습니다.

T자의 웨브는 "전단"(어긋나 밀리는 힘)으로 주저앉는데, 그 한계를
표준(KS ISO 12215-5)에서 그대로 옮겨 T자 뼈대를 자동 설계하도록
했습니다. 검사해보니 표준의 미리 계산된 값(강철 50·알루미늄 40)을
정확히 재현했고, 백지 심판도 물리 오류 0으로 통과시켰습니다.
이제 우리 배의 뼈대가 실선을 닮았습니다.

### 8/12 (38일차 종장) — 어림값 하나를 진짜 계산으로

배의 허리 단면 강도를 계산할 때, 뼈대(보강재)가 얼마나 강도에
보태는지를 그동안 **"배 길이의 0.3배 넓이"라는 어림값**으로
때웠습니다 — AI 심판이 여러 번 "이 숫자 근거 없다"고 지적한
부분입니다. 방금 진짜 T자 보강재를 설계할 수 있게 됐으니, 그
어림값을 **실제로 설계된 보강재의 단면적**으로 바꿨습니다.

바닥은 물 압력을 세게 받으니 바닥 뼈대가 갑판 뼈대보다 크게
나옵니다(물리 그대로). 어림값(넓이 30)보다 실제 바닥 뼈대가
조금 작아서, 그만큼 갑판 판을 살짝 두껍게 해서 강도를 맞췄습니다
— 정직한 되먹임입니다. 백지 심판도 5개 검사 축 전부 통과시켰고,
이걸로 구조 계산의 마지막 어림 상수 하나가 사라졌습니다.

### 8/12 (마무리) — 판이냐 뼈대냐, 같은 재료라도 기준이 다르다

국제표준(KS ISO 12215-5)의 설계응력 표를 받아보니, 같은 알루미늄
이라도 **판(외피)과 뼈대(보강재)의 허용 기준이 다르다**는 걸
알았습니다. 어제 우리가 넣은 88은 뼈대용 값이었고, 배 전체가
휘는 것(종강도)을 볼 때는 판 기준인 **112.5**를 써야 합니다.
즉 어제 배를 필요 이상으로 두껍게 만들고 있었던 것 — 25% 완화.

유리섬유(FRP)도 정본으로 세웠습니다. 표준의 계산표에서 손적층
매트(유리 30%) 물성을 그대로 가져와 탄성계수 8267·설계응력
69.75로 확정 (기존 추정 100·15000은 낙관·과대였음 확인). 등급도
"추정 C"에서 "정본 B"로 승격.

백지 심판이 이번에도 다섯 개를 잡았습니다. 특히 **유리섬유의
주저앉음(좌굴)을 계산할 때 "휘는 강도"를 쓰고 있었는데 "눌리는
강도"가 맞다**는 지적 — 물리적으로 정확한 교정입니다. 또 하나:
재료 이름으로 규정을 고르던 방식(강철이면 A규정, 아니면 B규정)을
**재료가 스스로 규정을 선언하는 방식**으로 바꿨습니다. 나중에
새 강종을 추가했을 때 조용히 잘못된 규정이 적용되는 사고를
막는 장치입니다.
...
**참고 숫자** (이해가 먼저, 숫자는 곁들임): 자동 검증 시험 454개 전부 통과 ·
**여덟 관문** 완성 (뜨나·안 넘어지나·실리나·안 흔들리나·안 부러지나·
돌 수 있나·규제·탄소성적) · 가상 바다 완주 조타 3구성 · 진화(NSGA)로
수만 척을 거른 완전 검증 전선 (좌굴 반영 v5 = 226척 전원 합격 신챔피언) · 실선
데이터 19척 + 모터 5종 + 방향타·서보 공식 스펙 · 로컬 시연 앱
(라이브 설계·갤러리) · 정밀해석(CFD) 20여 회 · 코드·기록 전부 GitHub 공개

**다음에 할 일 후보**: v5 재진화 결산 (좌굴로 배가 어떻게 무거워졌나) /
쌍동선 간섭 저항 원전(Molland — 봇 차단, 오너 브라우저 대기) / 실선
데이터 확충 (작업용 배수량형·전기 순찰정) / 내항 프록시(Jensen 닫힌식)
