...
> (마지막 갱신: 2026-08-03 — 오너 지침 반영 개편: 친구 눈높이·그림·설명 중심)
...
| **LCB (부력중심)** | 물이 배를 떠받치는 힘의 앞뒤 중심점. 무게중심(LCG)이 이 위에 있어야 수평으로 뜸 — 어긋난 만큼 트림 발생 |
...
**참고 숫자** (이해가 먼저, 숫자는 곁들임): 자동 검증 시험 267개 전부 통과 ·
가상 바다 완주 2척 × 조타 3구성 · AI 학습 라벨 1만 장 (실전 합격률 77%) ·
실선 데이터 12척 + 화물 실측 3점 + 모터 5종 + 방향타·서보 공식 스펙 ·
정밀해석(CFD) 실행 20여 회 · 실패 박물관 23건 · 코드·기록 전부 GitHub 공개

**다음에 할 일 후보** (오너 결정 순서): AI 특징 v3 (요약 정보의 질 —
데이터 양의 수확 체감 확정 후 남은 지렛대) / 가상 바다 미세 조사
(무타각 편향·활주 방향타 게인) / 실선 데이터 확충 / **그 뒤에** ELO
대결 재개 (시뮬 자산 다 쌓인 뒤 몰아보기 — 오너 결정)
