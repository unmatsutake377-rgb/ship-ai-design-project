...
6. [7일의 여정 — 날짜별 이야기](#6-7일의-여정--날짜별-이야기)
...
가상 바다에 띄워 스스로 항해까지 시켜보는 시스템**을 학부 2학년 학생과 AI가 7일 동안 함께 만들었습니다.
...
이날 실선 데이터도 3척 추가 (L30 제원 오류를 공식 PDF로 정정 — patrol 통계가 실측 기반으로 전환).
...
1. **정답지 있는 검증**: 모든 물리 코드는 "손으로 계산할 수 있는 문제"(상자 바지선 등)의
   정답과 대조하는 시험을 통과해야 합병. 현재 시험 206개
...
| ⑤ CFD | ✅ 훅 완성 + 실증 — 격자 수렴 연구 진행 중 |
| ⑥ 사람 피드백 학습 | ✅ ELO 랭킹 + 대리모델 + 능동 학습 1회전 (보정은 격자 수렴 후) |

**숫자로 보는 현재**: 시험 208개 전부 통과 · 코드는 전부 GitHub 공개 ·
가상 바다 완주 1회 · CFD 라벨 12행 · 실선 데이터 12척 (제조사 공식 스펙만) · ELO 대결 이력 축적 중

**다음에 할 일 후보**: 격자 2배 결과 판독(진행 중) → 보정 공식 재도전 /
실선 데이터 확충 / 최적화에도 공간 검사 연결 / 활주정 Gazebo 검증
